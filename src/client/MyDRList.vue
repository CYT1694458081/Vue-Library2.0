    ...
    <template >
      <div class="tb">
        <el-input style="box-shadow: 0 2px 4px rgba(0, 0, 0, .12), 0 0 6px rgba(0, 0, 0, .04)"
          placeholder="输入书名或原因" v-model="search">
    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <el-button
            size="mini"
            @click="delDR(scope.$index, scope.row)" type="success"  plain round>支付罚单</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    fetchData () {
      this.list = []
      postRequest('/api/public/getNowDRListByName', {name: JSON.parse(window.localStorage.getItem('user')).name}).then((result) => {
    ...
    delDR (index, row) {
      postRequest('/api/public/delDR/', {id: row.log_id}).then((result) => {
        console.log(result)
        if (result.data.code === 200) {
    ...
    find () {
      this.loading = true
      this.currentPage1 = 1
      postRequest('api/public/searchUserNowDRListByKey', {
